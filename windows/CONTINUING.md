...
- **Toasts are timed in-process.** `AppNotificationManager` cannot schedule, so `ExpiryNotifier` keeps its own timer. Approval, quick-activate and access-package event toasts go through the same `NotifyAsync`; package expiries (`SetPackageExpiriesAsync`) share the timer as keyless entries with no button.
