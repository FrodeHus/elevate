...
| Access packages: scope, models, provider, diff, new-role tracker, per-tenant state records (shared by the CLI and the app) | `src/Elevate.Core/Providers/AccessPackageProvider.cs`, `Coordination/AccessPackageDiff.cs`, `Coordination/NewRoleTracker.cs`, `Storage/AppState.cs` |
| Access packages in the app: polling (panel open, 15-minute throttle, 8-hour tick), event toasts, timed package-expiry toasts, the new-role marker, the window with four tabs, the request dialog, the tenant glyph and menu item | `src/Elevate.App.Model/ViewModels/AppModel.AccessPackages.cs`, `src/Elevate.App/Views/AccessPackagesWindow.xaml`, `RequestPackageWindow.xaml`, `Notifications/ExpiryNotifier.cs` |
| App model: accounts, refresh, activation, panel, profiles, approvals, operations, access packages | `src/Elevate.App.Model/ViewModels/AppModel.*.cs`, `tests/Elevate.App.Tests` (63 tests) |
...
- To exercise the flyout without signing in, back up `%LOCALAPPDATA%\Elevate\state.json` and `settings.json`, then seed `state.json` with an own-app identity, tenants in `manualRoles` mode, `manualRoles` entries and profiles (the golden fixture `tests/Elevate.Core.Tests/Fixtures/state-macos.json` shows the shape) and clear `clientId` in `settings.json`. Own-app identities are reconciled against the MSAL cache only when a client id is set, and first-party ones always are; an identity the cache does not know is kept but flagged `SignInNeeded` (refreshes skip it, the row offers **Sign in**), so seeded accounts still show up. Durations in `state.json` are Swift's `[high, low]` 128-bit attosecond pair, not seconds.
...
- **`AppModel` is one partial class in nine files** mirroring the Swift extensions (`Accounts`, `Activation`, `Refresh`, `Panel`, `Profiles`, `Approvals`, `Operations`, `AccessPackages`). It is single-threaded on the UI thread: it captures the constructing thread's `SynchronizationContext` and marshals coordinator progress, network changes and the hot key through `Post`. Views redraw on the coarse `Changed` event; the flyout reconciles its row objects by key so the list keeps its scroll position.
...
- **Expiry toasts are scheduled with the OS.** `ExpiryNotifier` builds the payload with `AppNotificationBuilder` and hands it to `ToastNotificationManager.CreateToastNotifier(aumid).AddToSchedule(new ScheduledToastNotification(...))` (in-box `Windows.UI.Notifications`, no extra package), so the toast fires when Elevate is not running and a button click launches it through the COM activator `AppNotificationManager.Register()` set up; the launch arrives on the existing `AppInstance.GetActivatedEventArgs()` AppNotification path. The SDK derives the unpackaged AUMID from a hash of the executable path and does not expose it, so `NotificationAppId` reads it back from `HKCU\Software\Classes\AppUserModelId\*` by matching the `CustomActivator` CLSID's `LocalServer32` command to `Environment.ProcessPath` (each build output path registers its own AUMID). Every refresh reconciles the OS schedule by `Tag` within the `elevate-expiry` group (`ExpiryPlan.Reconcile`: remove stale or moved, add missing), so an extended role drops its old toasts. Whatever the schedule refuses, or everything when it is unavailable, goes to the old in-process timer; a toast lives in exactly one of the two. `Dispose` calls `Unregister`, not `UnregisterAll`: the latter would delete the activator and orphan the scheduled toasts. Approval, quick-activate and access-package event toasts still go through `NotifyAsync` at once; package expiries (`SetPackageExpiriesAsync`) are scheduled the same way, keyless and without a button. Windows drops a scheduled toast whose delivery time passed while the machine was off for more than five minutes. `--test-toast <seconds>` schedules one Extend toast for verification.
- **`AppWindow.Size` includes the invisible resize frame.** The flyout keeps a border (`SetBorderAndTitleBar(true, false)`), so sizing the outer rectangle to 380 logical pixels left the client area one frame narrower and clipped the row buttons on the right. `FlyoutWindow.PixelSize` adds the measured `Size - ClientSize` difference; do the same for any window whose content is laid out at a fixed width.
- **Access packages are polled, never refreshed with the roles.** `PanelOpened` polls tenants with `AccessPackagesAvailable == true` at most every 15 minutes, an 8-hour `PeriodicTimer` polls in the background, and the window forces a poll on open, tab change and refresh. The availability flag is set by `ProbeAccessPackagesAsync` during the Entra read of a refresh, from the token's `scp` claim; `MsalTokenProvider.SignInScopes` asks for the entitlement scope with `User.Read` so a fresh sign-in carries it.
...
- **`BuildInfo`** takes the version from the assembly's informational version (`installer/build.ps1` sets it from the tag) and the signing state from the executable's Authenticode signature: `Signed: <certificate subject>`, which release builds have shown since 1.2.2 ("Open Source Developer Frode Hus", the Certum certificate), and "Unsigned" for a local build.
...
- **The winget manifest is generated, not committed, and submitted by the publish job** (`scripts/submit-winget.sh`, needs the `WINGET_TOKEN` secret): one microsoft/winget-pkgs pull request per package and release. winget-pkgs nests dotted identifiers, so the CLI's manifests live at `manifests/r/Reothor/Elevate/CLI/<version>`.
...
- **Issue #17, code signing and winget submission**: done. Signing with the Certum SimplySign certificate needs the `CERTUM_USERNAME`, `CERTUM_OTP_URI` and `CERTUM_KEY_ID` secrets (`docs/releasing.md`, "Windows signing secrets"; `scripts/Connect-SimplySign.ps1` opens the cloud session on the runner); winget submission needs `WINGET_TOKEN`. The first submissions were opened on 2026-09-15 and Microsoft published them: `Reothor.Elevate` and `Reothor.Elevate.CLI` at 1.7.0, `Reothor.Elevate.Audit` at 1.0.1. Releases are shared with macOS since 1.2.0: one `v*` tag, one release, `.github/workflows/release.yml`.
