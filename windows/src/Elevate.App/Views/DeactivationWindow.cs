...
{
    private sealed record Row(ActiveAssignment Assignment, TextBlock Status, ActivationStatusIcon Icon)
    {
    ...
    private DeactivationWindow(AppModel model, string name, IEnumerable<ProfileRun.Entry> entries, DateTimeOffset? startedAt, Guid? profileId = null)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            _rows.Add(new Row(entry.Assignment, status, icon));
        }
    ...
    private void Refresh()
    {
        ...
        {
            row.Status.Text = row.Completed ? "Deactivated" : row.Error ?? _model.DeactivationRefusal(row.Assignment) ?? "Ready to deactivate";
            row.Status.Foreground = (Brush)Application.Current.Resources[row.Completed ? "SystemFillColorSuccessBrush"
                : row.Error is null ? "TextFillColorSecondaryBrush" : row.Finished ? "SystemFillColorCautionBrush" : "SystemFillColorCriticalBrush"];
            ToolTipService.SetToolTip(row.Status, row.Status.Text);
        }
        _submit.IsEnabled = _model.IsOnline && _rows.Any(r => !r.Finished && !_model.InFlight.Contains(r.Assignment.RoleKey));
    }
    ...
    private async Task SubmitAsync()
    {
        ...
        foreach (var row in _rows.Where(r => !r.Finished))
        {
            row.Error = null;
