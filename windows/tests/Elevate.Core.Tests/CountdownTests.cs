...
public class CountdownTests
{
    ...
    [Fact]
    public void LabelUsesUnitsNotClockTime()
    {
        // "02:41" read as a time of day; units cannot.
        Countdown.Label(TimeSpan.FromSeconds(2 * 3600 + 41 * 60 + 10)).Should().Be("2 h 41 min");
        Countdown.Label(TimeSpan.FromSeconds(59)).Should().Be("< 1 min");
        Countdown.Label(TimeSpan.FromSeconds(5 * 60)).Should().Be("5 min");
        Countdown.Label(TimeSpan.FromSeconds(3600)).Should().Be("1 h");
        Countdown.Label(TimeSpan.FromSeconds(8 * 3600)).Should().Be("8 h");
    }
    ...
    {
        Countdown.Until(Now.AddSeconds(2 * 3600 + 15 * 60), Now).Should().Be("2 h 15 min");
        Countdown.Until(Now.AddSeconds(15 * 60 + 59), Now).Should().Be("15 min");
