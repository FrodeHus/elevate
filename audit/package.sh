...
#   audit/package.sh archive <version> <rid>   wraps it as audit/dist/elevate-audit-<version>-<rid>.tar.gz
...
STAGE="$ROOT/dist/$RID"
NAME="elevate-audit-$VERSION-$RID"

case "$RID" in
  win-*) EXE="elevate-audit.exe" ;;
  linux-*|osx-*) EXE="elevate-audit" ;;
...
case "$STEP" in
  publish)
    rm -rf "$STAGE"
    dotnet publish "$ROOT/src/Elevate.Audit/Elevate.Audit.csproj" -c Release -r "$RID" -o "$STAGE" -p:Version="$VERSION" --nologo
