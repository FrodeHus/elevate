...
@Suite struct AzureApprovalProviderTests {
    ...
    @Test func listsOnlyPendingApprovalRequests() async throws {
        ...
        #expect(items.map(\.id) == ["areq-1", "areq-2", "areq-4", "areq-5"])
        #expect(items.map(\.action) == [.activate, .extend, .renew, .other])
        #expect(items.map(\.requesterName).prefix(2) == ["Ann Approver", "user-obj-2"])
        #expect(items.map(\.scopeCaption).prefix(2) == ["rg-ops · resource group", "Contoso Production · subscription"])
        #expect(items.map(\.decisionRef) == ["appr-1", "appr-2", "appr-4", "appr-5"])
