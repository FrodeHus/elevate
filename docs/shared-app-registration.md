...
Switching later is a one-field change: put your own client id into Settings (or push it). Accounts
that used the old one keep their tenants, roles and profiles and show **Sign in** so they can sign
in again with the new id; accounts with their own registration, and Azure CLI and Azure PowerShell
