...
| Delegated permissions | Exactly the ones in [the permission table](entra-app-registration.md#4-permission-table), plus `openid`, `profile` and `offline_access` |
...
Administrator (for the assignment) or a Global Administrator. Until it is granted, an account in
that tenant signs in but shows **manual roles**, because Elevate cannot read eligibility.

### From the app

Elevate offers the shared app and its consent link in four places:

- The **setup panel** on first launch: **Quick start with the shared Elevate app…**, then
  **Grant admin consent…**. (The other buttons are **Open Settings…** for your own or your
  company's registration, and **Continue with the Azure CLI app**.)
- The **No accounts** state, once the shared app is configured.
...
- The **tenant menu** on any signed-in tenant: **Open admin consent link…**, pre-filled for that
  tenant.
