...
- **No app registration needed.** Graph calls use Microsoft's *Microsoft Graph
  ...
  with dynamic consent to five read-only scopes; Azure calls use the Azure CLI
...
|---|---|---|
| Microsoft Graph | `14d82eec-204b-4c2f-b7e8-296a70dab67e` (default) | `RoleManagement.Read.Directory`, `PrivilegedAssignmentSchedule.Read.AzureADGroup`, `PrivilegedEligibilitySchedule.Read.AzureADGroup`, `GroupMember.Read.All`, `User.ReadBasic.All` |
...
- `GraphTransport` already retries on 429 honouring `Retry-After`; the ARM
  helper gets the same treatment if it lacks it.
...
Prints to PDF cleanly (`@media print` collapses `<details>` open). Dark mode
via `prefers-color-scheme`, matching the site.
...
- `site/audit.html`: what the tool finds, the five Graph scopes with one line
  ...
  screenshot of the terminal output, and a link to `site/audit-sample.html`.
...
- Homebrew: `Formula/elevate-audit.rb`, generated by extending
  `scripts/update-formula.sh` to take an asset prefix and formula name.
...
- **Rules** on hand-written snapshot fixtures: direct user; group with two
