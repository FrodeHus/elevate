...
The audit tool is not part of that release. `elevate-audit` has its own version
line, its own changelog (`audit/CHANGELOG.md`), `audit-v<x.y.z>` tags and its own
workflow, `.github/workflows/release-audit.yml`, so a change to it does not
rebuild and re-sign the apps, and an app release does not republish it. See
[Releasing the audit tool](#releasing-the-audit-tool).
...
3. The tag push starts a second Release run, which builds, publishes and
   ...
   the cask and formula bump is on `main`, and the winget pull requests for
   `Reothor.Elevate` and `Reothor.Elevate.CLI` are open in `microsoft/winget-pkgs`
   (the run summary links them; the generated manifests are also workflow
   artifacts named `winget-manifest` and `winget-cli-manifest`).
...
the signing certificates, the release and the deploy key. The audit tool's
`audit-v*` tags are not covered by the `release` ruleset unless its pattern is
extended to `refs/tags/audit-v*`; do that so those tags cannot be moved either.
...
5. Runs `scripts/update-cask.sh` and `scripts/update-formula.sh` on a checkout
   ...
   once or twice if main moved meanwhile. `Formula/elevate-audit.rb` is left
   alone; the audit release workflow owns it. The cask's `caveats` block is
   included only for unsigned builds.

Both winget manifests are submitted by `scripts/submit-winget.sh`, right after
the release is created and before the cask commit: one pull request per package
to `microsoft/winget-pkgs`, from the `winget-pkgs` fork of the account whose
token is in `WINGET_TOKEN` (see [winget submission](#winget-submission)).
Microsoft's pipeline then validates and installs the package and a moderator
merges, usually within a day or two; `winget install Reothor.Elevate` works
once the merge is published to the source. A release without the token skips
the step with a notice, and the artifacts can be submitted by hand with the
same script.

## Releasing the audit tool

`elevate-audit` is released by `.github/workflows/release-audit.yml`, a smaller
copy of the app workflow with its own version line. Its tags are
...
3. The tag push starts a second run: `check` (tag on `main`, changelog section
   present), then the same three-leg `audit` matrix as before (tests from
   `audit/Elevate.Audit.sln`, `audit/package.sh publish|archive`, the same
   signing and notarization steps as the CLI, archives named
   `elevate-audit-<version>-<rid>`, the `Reothor.Elevate.Audit` winget manifest
   as the `winget-audit-manifest` artifact), then `publish`, which writes
   `elevate-audit-<version>-checksums.txt`, creates the release with the
   archives and the changelog section as notes, submits the winget manifest
   (`scripts/submit-winget.sh`, see [winget submission](#winget-submission)),
   and commits
   `Formula/elevate-audit.rb`, regenerated by `scripts/update-audit-formula.sh`,
   to `main` as "Formula: Elevate Audit x.y.z" over the deploy key.

The release is created with `--latest=false`, so `releases/latest`, the README
badge and the app's update check keep pointing at the app's newest release no
matter which line published last. The audit tool's downloads are found under
[releases?q=audit-v](https://github.com/FrodeHus/elevate/releases?q=audit-v);
the formula and the winget manifest download from
`releases/download/audit-v<version>/`.

Pushing an `audit-v*` tag by hand works the same way as for the app: the tag
must point at a commit on `main` whose `audit/CHANGELOG.md` already has the
section. To redo an audit release, delete the tag and the release and push the
tag again.

The first cut after the split compares against `v1.6.7` rather than a
non-existent `audit-v1.6.7`: the cut script takes the base of the new
comparison link from the changelog's existing `[Unreleased]` link, and later
cuts chain from the previous `audit-v` tag.
...
The audit tool's formula, `Formula/elevate-audit.rb`, is the same shape and is
regenerated by `scripts/update-audit-formula.sh <version> <owner/repo> <dist dir>`
from the audit release workflow, never from the app's. Its download URLs point
at the `audit-v<version>` release:
...
`scripts/submit-winget.sh <manifest-dir> <package-id> <version>` opens the
`microsoft/winget-pkgs` pull request for one generated manifest set. It runs in
the publish job of both release workflows with the artifacts the build jobs
validated, and needs one secret:

| Secret | What it is |
|---|---|
| `WINGET_TOKEN` | A classic personal access token with the `public_repo` scope for the GitHub account that owns (or will own) a fork of `microsoft/winget-pkgs`; the account must have signed Microsoft's CLA once. A fine-grained token cannot open pull requests against a repository it does not own, and the deploy key has no GitHub API access |

The script forks `winget-pkgs` if the account has no fork, updates the fork's
`master`, creates the branch `<package-id>-<version>`, writes the manifests
through the contents API at the path winget-pkgs demands (every dot-separated
segment of the identifier is a directory level: `manifests/r/Reothor/Elevate/CLI/1.7.0`)
and opens the pull request titled "New package: … version …" or "New version: …
version …". An existing branch is updated; an open pull request for the same
head is left alone, so a re-run of the publish job is safe. winget-pkgs wants
one package per pull request, which is why the app and the CLI go separately.

Do not delete a release while its pull requests are validating: the pipeline
fetches the installer URLs, and a missing asset fails the "URLs Validation"
step until the pull request is re-pushed.

## Windows signing secrets

Without them the Windows and CLI jobs here, and the audit release workflow,
publish unsigned Windows binaries. With all three secrets they sign
`elevate.exe`, `elevate-audit.exe` and the MSIs with a
